...
parser.add_argument('--loadckpt', default='./pretrained_model/train1.2.ckpt',help='load the weights from a specific checkpoint')
# parse arguments
...
save_dir = '/home/jyhy/ACVNet-main/test/'
    ...
    for batch_idx, sample in enumerate(TestImgLoader):
      if(a<90):
        ...
        for disp_est, fn in zip(disp_est_np, left_filenames):
            ...
            dev = disp_est-disp_gt_np[0]
            disp_est = np.array(disp_est, dtype=np.float32)
            #b=np.ones((len(disp_est),len(disp_est[0])))*35
            #depth=b/disp_est
            fn = os.path.join(save_dir, fn.split('/')[-4]+fn.split('/')[-3]+fn.split('/')[-1])
            print("saving to", fn, disp_est.shape)
            disp_est_uint = np.round(disp_est * 256).astype(np.uint16)
            skimage.io.imsave(fn, disp_est_uint)
            #print("saving to", fn, depth.shape)
            #depth_to = np.round(depth * 256).astype(np.uint16)
            #skimage.io.imsave(fn, depth_to)
            ...
            dev = np.array(dev, dtype=np.float32)
            fn = os.path.join(save_dir, 'hhh'+fn.split('/')[-4]+fn.split('/')[-3]+fn.split('/')[-1])
            print("saving to", fn, dev.shape)
            #dev = np.round(dev*0.2 ).astype(np.uint16)
            #dev = (dev*256).astype(np.uint8)
            skimage.io.imsave(fn, dev)
